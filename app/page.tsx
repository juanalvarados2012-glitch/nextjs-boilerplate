...
function Results({ kit, form, onRestart, onNewBrand, callAI, isPremium, onLogin, userEmail, onLogout, onOpenLogin, savedKits = [], currentKitId, onUpdateKitContent, onLoadKit, onDeleteKit }: any) {
  ...
  const downloadAll = () => {
    const lines: string[] = [`${form.name} — Full Content Kit`, `Generated by BrandMind`, `${"=".repeat(50)}`, ""];
    lines.push("BRAND IDENTITY", "-".repeat(30));
    lines.push(`Taglines:\n${(kitData.taglines || []).map((t: string, i: number) => `  ${i + 1}. ${t}`).join("\n")}`);
    lines.push(`\nBrand Voice:\n  ${kitData.brandVoice || ""}`);
    lines.push(`\nBrand Story:\n  ${kitData.brandStory || ""}`);
    lines.push(`\nColor Palette:\n${(kitData.colors || []).map((c: any) => `  ${c.name}: ${c.hex} (${c.role})`).join("\n")}`);
    lines.push(`\nTypography:\n  Display: ${kitData.typography?.display?.name || ""}\n  Body: ${kitData.typography?.body?.name || ""}`);
    if (allContent.posts?.length) {
      lines.push("", "=".repeat(50), "SOCIAL MEDIA POSTS", "-".repeat(30));
      allContent.posts.forEach((p: any, i: number) => {
        lines.push(`\nPost ${i + 1}:\n"${p.hook}"\n\n${p.caption}\n\n${p.hashtags}`);
      });
    }
    if (allContent.copy?.hero) {
      lines.push("", "=".repeat(50), "WEBSITE COPY", "-".repeat(30));
      const c = allContent.copy;
      lines.push(`\nHero:\n  Headline: ${c.hero?.headline}\n  Subheadline: ${c.hero?.subheadline}\n  CTA: ${c.hero?.cta}`);
      lines.push(`\nAbout:\n  ${c.about?.headline}\n  ${c.about?.body}`);
      if (c.services?.items) lines.push(`\nServices:\n${c.services.items.map((s: any) => `  • ${s.name}: ${s.desc}`).join("\n")}`);
      if (c.social_proof) lines.push(`\nSocial Proof:\n  "${c.social_proof}"`);
    }
    if (allContent.reels?.length) {
      lines.push("", "=".repeat(50), "REEL SCRIPTS", "-".repeat(30));
      allContent.reels.forEach((r: any, i: number) => {
        lines.push(`\nReel ${i + 1}: ${r.title} (${r.duration})\n  HOOK: ${r.hook}\n  BODY: ${r.body}\n  CTA: ${r.cta}`);
      });
    }
    if (allContent.bio?.instagram) {
      lines.push("", "=".repeat(50), "PLATFORM BIOS", "-".repeat(30));
      const b = allContent.bio;
      if (b.instagram) lines.push(`\nInstagram:\n  ${b.instagram.bio}\n  Link CTA: ${b.instagram.link_cta}`);
      if (b.tiktok) lines.push(`\nTikTok:\n  ${b.tiktok.bio}`);
      if (b.linkedin) lines.push(`\nLinkedIn:\n  ${b.linkedin.headline}\n  ${b.linkedin.summary}`);
      if (b.twitter) lines.push(`\nTwitter/X:\n  ${b.twitter.bio}`);
    }
    const blob = new Blob([lines.join("\n")], { type: "text/plain" });
    const a = document.createElement("a");
    a.href = URL.createObjectURL(blob);
    a.download = `${form.name.replace(/ /g, "-")}-brand-kit.txt`;
    a.click();
    ...
    <div style={{ background: BG, minHeight: "100vh" }}>
      <div style={{ position: "sticky", top: 0, zIndex: 50, background: "rgba(7,8,9,.97)", backdropFilter: "blur(20px)", borderBottom: "1px solid rgba(255,255,255,.05)", padding: "10px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "10px" }}>
        ...
        <div style={{ display: "flex", gap: "7px", alignItems: "center" }}>
          ...
          {isPremium && genStatus === "done" && <button onClick={downloadAll} className="o" style={{ padding: "7px 12px", fontSize: "12px" }}>⬇ Download All Content</button>}
